...
def CreateWidgets():
    saveFileLabel = Label(root, text='FILE NAME : ', font=('', 15, 'bold'), bg='#d5d9e0')
    saveFileLabel.grid(row=0, column=0, padx=5, pady=5)

    saveFileEntry = Entry(root, width=50, textvariable=fileName, bg='white')
    ...
    fileTextLabel = Label(root, text='FILE TEXT : ', font=('', 15, 'bold'), bg='#d5d9e0')
    fileTextLabel.grid(row=1, column=0, padx=5, pady=5)

    root.fileTextEntry = Text(root, width=120, height=30, bg='white')
...
def fileBrowse():
    ...
    f_name = filedialog.asksaveasfilename(initialdir='/Users',
...
root.title('Hemang_Notepad')
root.geometry('1000x650')
root.resizable(True, True)
root.configure(background='#d5d9e0')
